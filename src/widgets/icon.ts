...
export type IconProps<Attr = unknown, Self = Icon<Attr>> = BaseProps<
  Self,
  Gtk.Image.ConstructorProperties & {
    size?: number;
    icon?: Ico;
  },
  Attr
...
export function newIcon<Attr = unknown>(
...
export class Icon<Attr> extends Gtk.Image {
  ...
  set icon(icon: string | GdkPixbuf.Pixbuf) {
    ...
    if (typeof icon === "string") {
      ...
      console.warn(
        Error(`expected Pixbuf or string for icon, but got ${typeof icon}`),
      );
      this._set("icon", "image-missing-sybmolic");
      this._set("type", "named", false);
    }
